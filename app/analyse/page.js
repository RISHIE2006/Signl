...
import { Sparkles, AlertCircle, BrainCircuit, Lock } from "lucide-react";
...
export default function AnalysePage() {
  const { user } = useUser();
  const { canAddAnalysis, limits } = useBilling();
    ...
    try {
      ...
      if (!res.ok) {
        ...
        try {
          ...
          setError(
            `Server error ${res.status}. Please check your connection or try again.`,
          );
        }
        return;
      }
  ...
  const generateLearningPath = async () => {
    ...
    try {
      const res = await fetch("/api/learning-path", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          skills: result.skillGaps,
          role: "the target role",
        }),
      });
      if (!res.ok) {
        const text = await res.text();
        console.error('Learning path request failed:', res.status, text);
        return;
      }

      const data = await res.json();
      if (!Array.isArray(data)) {
        console.error('Learning path: unexpected response format', data);
        return;
      }
        ...
        <div className="page-container">
          ...
          >
            ...
            <p className="page-subtitle">
              Upload your resume and the job description to find the gap.
            </p>
          </motion.div>
          ...
          {view === "tailored" && tailoredData ? (
            ...
            <>
              ...
              <motion.div
                ...
                className="grid-2"
                style={{ marginBottom: "24px", alignItems: "start" }}
              >
              ...
              >
                <LoadingButton
                  ...
                  onClick={analyse}
                  isLoading={loading}
                  disabled={!resumeData?.text || !jd.trim() || !canAddAnalysis}
                  style={{
                    padding: "12px 40px",
                    opacity:
                      !loading &&
                      (!resumeData?.text || !jd.trim() || !canAddAnalysis)
                        ? 0.4
                        : 1,
                  }}
                >
                ...
                <AnimatePresence>
                  {loading && (
                    ...
                    >
                      ...
                      Please wait for 15 to 20 seconds while our AI analyzes
                      your resume...
                    </motion.div>
                  )}
                </AnimatePresence>
                {!canAddAnalysis && (
                  ...
                  >
                    You have reached your limit of {limits.analysesLimit} free
                    analyses.{" "}
                    <a
                      href="/billing"
                      style={{
                    ...
                    <div className="grid-2">
                      ...
                      <div className="card">
                        ...
                        {result.skillGaps?.length > 0 && (
                          <div style={{ marginTop: "24px" }}>
                            ...
                            >
                              <BrainCircuit size={14} /> Bridge the Gap (AI
                              Learning Path)
                            </LoadingButton>
                      ...
                      >
                        ...
                        <div style={{ marginTop: "30px" }}>
                          ...
                          >
                            <Sparkles size={16} /> Redesign & Tailor for this
                            Job (Instant)
                          </button>
                          <p
                            style={{
                          ...
                          >
                            Your tailored content was already generated during
                            analysis! Click to view your new redesigned profile.
                          </p>
                        </div>
                      </div>
                    </div>
